...
////                if (dungeon.rooms[i][j].sector == UNINITIALIZED) && (Double.random(in: 0.0..<1.0) < 0.7) {
...
//
//func generateSectors(_ dungeon: inout Dungeon) {
//dungeon.rooms[1][1].sector = 0
//dungeon.rooms[1][1].grid_i = 1
//dungeon.rooms[1][1].grid_j = 1
//dungeon.sequence[0] = dungeon.rooms[1][1]
//                    
//dungeon.rooms[1][2].sector = 1
//dungeon.rooms[1][2].grid_i = 1
//dungeon.rooms[1][2].grid_j = 2
//dungeon.sequence[1] = dungeon.rooms[1][2]
//                   
//dungeon.rooms[2][1].sector = 2
//dungeon.rooms[2][1].grid_i = 2
//dungeon.rooms[2][1].grid_j = 1
//dungeon.sequence[2] = dungeon.rooms[2][1]
//                    
//                    
//dungeon.rooms[2][3].sector = 3
//dungeon.rooms[2][3].grid_i = 2
//dungeon.rooms[2][3].grid_j = 3
//dungeon.sequence[3] = dungeon.rooms[2][3]
//                    
//                    
//dungeon.rooms[3][2].sector = 4
//dungeon.rooms[3][2].grid_i = 3
//dungeon.rooms[3][2].grid_j = 2
//dungeon.sequence[4] = dungeon.rooms[3][2]
//
//dungeon.roomCount = 5
//
//        dungeon.sequence.sort { room1, room2 in
...
//
//        let currentIndex = dungeon.sequence[i]!.grid_i
//        let currentJndex = dungeon.sequence[i]!.grid_j
//        let nextIndex = dungeon.sequence[i + 1]!.grid_i
//        let nextJndex = dungeon.sequence[i + 1]!.grid_j
//        
// 
//        var current = dungeon.rooms[currentIndex][currentJndex]
//        var next = dungeon.rooms[nextIndex][nextJndex]
//        
...
//        
//        // ОБНОВЛЕНИЕ
//        dungeon.rooms[currentIndex][currentJndex] = current
//        dungeon.rooms[nextIndex][nextJndex] = next
//    }
//}
//
///////**КОРИДОРЫ И ДВЕРИ **/
...
//        for j in 1..<ROOMS_PER_SIDE + 1 {
//
//            // Работаем с оригинальными объектами через индексы
//            let currentRoomIndex = (i, j)
//            let currentRoom = dungeon.rooms[i][j]
//            
//            if let rightRoom = currentRoom.connections[RIGHT] {
//                // Проверяем связь через оригинальные объекты
//                if dungeon.rooms[i][j].connections[RIGHT]?.connections[LEFT]?.sector == currentRoom.sector {
//                    var corridor = dungeon.corridors[dungeon.corridorsCount]
//                    
//                    // Передаем индексы комнат, а не копии
//                    generateLeftToRightCorridor(&dungeon, currentRoomIndex, (rightRoom.grid_i, rightRoom.grid_j), &corridor)
//                    
...
//            }
//            
////            if let bottomRoom = currentRoom.connections[BOTTOM] {
////                // Аналогично для вертикальных коридоров
...
////                    generateLeftTurnCorridor(&dungeon, currentRoomIndex, (bottomRoom.grid_i, bottomRoom.grid_j), &corridor)
////                } else if gridIDiff == -1 && gridJDiff < 0 {
////                    generateRightTurnCorridor(&dungeon, currentRoomIndex, (bottomRoom.grid_i, bottomRoom.grid_j), &corridor)
////                } else {
////                    generateTopToBottomCorridor(&dungeon, currentRoomIndex, (bottomRoom.grid_i, bottomRoom.grid_j), &corridor)
////                }
////                
////                dungeon.corridors[dungeon.corridorsCount] = corridor
////                dungeon.corridorsCount += 1
...
//}
//
//func generateLeftToRightCorridor(_ dungeon: inout Dungeon, _ leftRoomIndex: (Int, Int), _ rightRoomIndex: (Int, Int), _ corridor: inout Corridor) {
//    let leftRoom = dungeon.rooms[leftRoomIndex.0][leftRoomIndex.1]
//    let rightRoom = dungeon.rooms[rightRoomIndex.0][rightRoomIndex.1]
//    
...
//    
//    for i in 1..<ROOMS_PER_SIDE + 1 {
//        if dungeon.rooms[i][leftRoomIndex.1].sector != UNINITIALIZED && i != leftRoomIndex.0 {
//            xMin = max(dungeon.rooms[i][leftRoomIndex.1].botRight.x, xMin)
//        }
//    }
//    for i in 1..<ROOMS_PER_SIDE + 1 {
//        if dungeon.rooms[i][rightRoomIndex.1].sector != UNINITIALIZED && i != rightRoomIndex.0 {
//            xMax = min(dungeon.rooms[i][rightRoomIndex.1].topLeft.x, xMin)
//        }
//    }
//    
...
////            var currentRoom = dungeon.rooms[i][j]
////            
////            if dungeon.rooms[i][j].connections[RIGHT] != nil && dungeon.rooms[i][j].connections[RIGHT]?.connections[LEFT] == dungeon.rooms[i][j]
////            {
////                var rightRoom = dungeon.rooms[i][j].connections[RIGHT]!
////                var corridor = dungeon.corridors[dungeon.corridorsCount]
////                
////                generateLeftToRightCorridor(&dungeon, &currentRoom, &rightRoom, &corridor)
////                dungeon.corridors[dungeon.corridorsCount] = corridor
